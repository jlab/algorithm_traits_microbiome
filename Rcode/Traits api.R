...
# after that, the 02 Databases were merged
...
gold_DB <- gold_DB %>% 
  mutate(Species = gsub("\\[|\\]", "",Species),
        Species = gsub(' bacterium.*|uncultured.*', '',Species)) %>%
  separate(Species, c("Genus","Species"),fill = "right") %>%
  mutate(Species = gsub("sp", "", Species)) %>% 
  filter(!is.na(Genus),!is.na(Species),Species != "")
...
# convert val column in genos to numeric - before combining all data frames
...
#write_xlsx(trait_wide, 'C:\\Users\\Anwender\\Downloads\\Traits_wide.xlsx')
#library(reshape2)
trait_wide <- dcast(x, Genus + Species ~ trait, value.var="val")

################################
#+ merge search_taxon and gold db via common genus & species names
#View(left_join(search_taxon,gold_DB,by=c('Genus','Species'))%>% summarize(n=n()))
#+ grouping same genus & species together - will be used after digitizing traits 
#View(filter(gold_DB,Genus != "") %>% group_by(Genus,Species) %>% summarize(n=n(),.groups = 'keep'))

# sp <- gsub("[[]]", "",gold_DB$Species)
